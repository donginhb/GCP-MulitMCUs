...
/**
  ...
  * @retval 0-成功 非0-失败
  */
static uBit32 BS01_GetWeatherParmForJson(char const *pJsonPack, char *pKey, char *pValue)
{
    uBit32 ulRet = 0;
...
/**
  ...
  * @retval 0-成功 非0-失败
  */
static uBit32 BS01_GetBaseClockForJson(char const *pJsonPack, char *pKey, char *pValue)
{
    uBit32 ulRet = 0;
 ...
 * 网络参数接口获取解析相关接口
 ****************************************************************************/
#define BS01_CLOCK_UPDATE_TIME          (3 * 60 * 60 * 1000)    //更新时间(MS)
...
/**
  ...
  * @retval 0-成功 非0-失败
  */
uBit32 BS01_UpdateWeather(void)
{
    //从网络中获取最新的JSON包
    uBit32 ulRet = BS01_RecvWeatherPack(m_uWeatherJsonPack, &m_ulWeatherJsonLen);
    
    //判断获取结果
    if (ulRet == 0)
    {
        //解析数据包并提取参数
        if (BS01_GetWeatherParmForJson((char const *)m_uWeatherJsonPack, "name", (char *)&m_uCityName))
        {
            return 1;
        }
        
        if (BS01_GetWeatherParmForJson((char const *)m_uWeatherJsonPack, "text", (char *)&m_uWeather))
        {
            return 1;
        }
        
        if (BS01_GetWeatherParmForJson((char const *)m_uWeatherJsonPack, "temperature", (char *)&m_uTemp))
        {
            return 1;
        }
    }
...
/**
  ...
  * @retval 0-成功 非0-失败
  */
uBit32 BS01_UpdateClock(void)
{
    //从网络中获取最新的JSON包
    uBit32 ulRet = BS01_RecvClockPack(m_uClockJsonPack, &m_ulClockJsonLen);
    
    //判断获取结果
    if (ulRet == 0)
    {
        //解析数据包
        if (BS01_GetBaseClockForJson((char const *)m_uClockJsonPack, "timestamp", (char *)&m_uBaseClock))
        {
            return 1;
        }
        ...
        //设置下次网络同步的时间
        SysTime_StartOneShot(&m_UpdateClockTimer, BS01_CLOCK_UPDATE_TIME); 
...
/**
  ...
  * @retval None
  */
void BS01_UpdateClockHandler(void)
{
 ...
 * WIFI时钟显示相关接口
 ****************************************************************************/
#define BS01_WIFI_TOGGLE_TIME          (1000)    //翻转时间(MS)
...
/**
  ...
  * @retval None
  */
static void BS01_DisplayParm(void)
{
...
/**
  ...
  * @retval None
  */
void BS01_UpdateDisplay(void)
{
    if (SysTime_CheckExpiredState(&m_DisplayTimer))
    {
        //设置下一次执行的时间
        SysTime_StartOneShot(&m_DisplayTimer, BS01_WIFI_TOGGLE_TIME); 
