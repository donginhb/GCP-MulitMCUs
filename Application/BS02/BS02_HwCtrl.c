...
/**
  ...
  * @retval None
  */
static void BS02_IOConfig(void)
{
    //设置控制IO资源表
...
void BS02_HwInit(void)
{
    ...
    //初始化串口
    UART_Init(BS02_COM_UART_NODE, 115200);
 ...
 * LED显示线程接口
 ****************************************************************************/
#define BS02_LED_TOGGLE_TIME             (100)            //LED翻转时间(MS)
...
/**
  ...
  * @retval None
  */
void BS02_ShowMainWorkLed(void)
{
    if (SysTime_CheckExpiredState(&m_LedCtrlTimer))
    {
        SysTime_StartOneShot(&m_LedCtrlTimer, BS02_LED_TOGGLE_TIME); //设置下一次执行的时间
 ...
 * 按键扫描线程接口
 ****************************************************************************/
#define BS02_KEY_SCAN_INTERVAL       (50)         //按键扫描间隔(MS)
...
/**
  ...
  * @retval None
  */
void BS02_KeyProc(void)
{
    if (SysTime_CheckExpiredState(&m_KeyScanTimer))
    {
        SysTime_StartOneShot(&m_KeyScanTimer, BS02_KEY_SCAN_INTERVAL);   //设置下一次执行的时间
        ...
        if (ulCurTrg == 1)
        {
            BS02_SetMotorDirect(!BS02_GetMotorDirect());
        }
        else if (ulCurTrg == 2)
        {
            BS02_EnableMotor(!BS02_GetMotorEnableStatue());
        }
    }
