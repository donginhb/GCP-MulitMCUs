...
//系统报警字
#define GCP_ALARM_SYS_ALARM			(0x00000001)		//系统报警位
#define GCP_ALARM_CRD_ALARM			(0x00000002)		//通道报警位
#define GCP_ALARM_AXIS_ALARM			(0x00000004)		//轴报警位
#define GCP_ALARM_PO_ALARM			(0x00000008)		//系统掉电报警
#define GCP_ALARM_CONF_INVALID		(0x10000000)		//系统配置故障
#define GCP_ALARM_MOTOR_ENABLE       (0x20000000)        //电机使能失败
//#define GCP_ALARM_CRD_MODE         (0x40000000)        //坐标系模式设置错误
#define GCP_ALARM_CRDAXIS_MAP        (0x80000000)        //坐标系轴映射表错误
...
/**
  ...
  * @retval None
  */
static void GCP_MoveLiftMotorConfig(void)
{
    uBit32 ulRet;
    ...
    if(ulCurDevCount != 1)
    {
      m_VmAlarm |= GCP_ALARM_CONF_INVALID;
      return;
    }
    ...
    if(ulRet)
    {
       m_VmAlarm |= GCP_ALARM_CRDAXIS_MAP;
    }
...
//升降平台位置状态定义
#define GCP_LIFT_PLATFORM_IN_UNKNOWN                 (0)     //未知位置
#define GCP_LIFT_PLATFORM_IN_UP_LIMIT_SITE           (1)     //在上限位
#define GCP_LIFT_PLATFORM_IN_DOWN_LIMIT_SITE         (2)     //在下限位
#define GCP_LIFT_PLATFORM_IN_CENTRE_SITE             (3)     //在中间(上下限位之间)

static uBit32 m_ulLiftPlatformPosition = GCP_LIFT_PLATFORM_IN_UNKNOWN; //提升平台的位置
...
uBit32 GCP_MoveLiftMotor(Bit32 iMotorNO, POSCTRL_MOTION_DATA* pPosCtrlMotion)
{
    ...
    {
        return GCP_ERR_LIFT_MOTOR;
    }
    
	return GCP_ERR_SUCCEED;
}
...
void GCP_LiftPlatformHandler(void)
{
    ...
    if (m_ulLiftPlatformPosition == GCP_LIFT_PLATFORM_IN_UNKNOWN)
    {
        //查询上限位状态
        if (GPIO_GetInputState(INPUT_IO_LIFT_MOTOR_UP_LIMIT) == GCP_LIFT_MOTOR_TRIGGER_LEVEL)
        {
            m_ulLiftPlatformPosition = GCP_LIFT_PLATFORM_IN_UP_LIMIT_SITE;       //升降平台在上限位
        }
        //查询下限位状态
        else if (GPIO_GetInputState(INPUT_IO_LIFT_MOTOR_DOWN_LIMIT) == GCP_LIFT_MOTOR_TRIGGER_LEVEL)
        {
            m_ulLiftPlatformPosition = GCP_LIFT_PLATFORM_IN_DOWN_LIMIT_SITE;     //升降平台在上限位
        }
        else
        {
            m_ulLiftPlatformPosition = GCP_LIFT_PLATFORM_IN_CENTRE_SITE;         //升降平台在中间
        }
    ...
    //查询上限位状态
    if (GPIO_MAN_GetInputPinState(INPUT_IO_LIFT_MOTOR_UP_LIMIT) == GCP_LIFT_MOTOR_TRIGGER_LEVEL)
    {
        //假如先前的升降平台不在上限位上而当前到达上限位,则急停电机
        if (m_ulLiftPlatformPosition != GCP_LIFT_PLATFORM_IN_UP_LIMIT_SITE)
        {
            //设置当前升降平台位置
            m_ulLiftPlatformPosition = GCP_LIFT_PLATFORM_IN_UP_LIMIT_SITE;
    ...
    //查询下限位状态
    else if (GPIO_MAN_GetInputPinState(INPUT_IO_LIFT_MOTOR_DOWN_LIMIT) == GCP_LIFT_MOTOR_TRIGGER_LEVEL)
    {
        //假如先前的升降平台不在下限位上而当前到达下限位,则急停电机
        if (m_ulLiftPlatformPosition != GCP_LIFT_PLATFORM_IN_DOWN_LIMIT_SITE)
        {
            //设置当前升降平台位置
            m_ulLiftPlatformPosition = GCP_LIFT_PLATFORM_IN_DOWN_LIMIT_SITE;
    ...
    {
        //设置当前升降平台位置
        m_ulLiftPlatformPosition = GCP_LIFT_PLATFORM_IN_CENTRE_SITE;
    }
