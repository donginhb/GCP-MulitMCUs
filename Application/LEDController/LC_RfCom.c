 ...
 * 私有成员定义及实现
 ****************************************************************************/
#define LC_BUFF_LEN     (64)

#define LC_SET_LED_OPEN            "LED_OPEN."
#define LC_SET_LED_CLOSE           "LED_CLOSE."
#define LC_SET_LED_AUTO            "LED_AUTO."

static uBit8 uCmdBuff[LC_BUFF_LEN] = {0};   //定义指令缓冲区
...
void RF_ComHandler(void)
{
    ...
    if (RF_RecvBuff(&uCmdBuff[ulRxIndex], 1))
    {
        ...
        {
            //LED灯启动
            if (memcmp(uCmdBuff, LC_SET_LED_OPEN, strlen(LC_SET_LED_OPEN)) == 0)
            {
                //执行指令
            ...
            //LED灯关闭
            else if (memcmp(uCmdBuff, LC_SET_LED_CLOSE, strlen(LC_SET_LED_CLOSE)) == 0)
            {
                //执行指令
            ...
            //LED灯自动控制
            else if (memcmp(uCmdBuff, LC_SET_LED_AUTO, strlen(LC_SET_LED_AUTO)) == 0)
            {
                //执行指令
