...
{
    unsigned char *ptr = RingBuff->data;

    /* We cannot insert when queue is full */
    if (RingBuff_IsFull(RingBuff))
        return 0;
...
int RingBuff_InsertMult(RINGBUFF_T *RingBuff, const void *data, int num)
{
    ...
    cnt1 = cnt2 = RingBuff_GetFree(RingBuff);
    ...
    /* Write segment 2 */
    ptr = (unsigned char *) RingBuff->data + RB_INDH(RingBuff) * RingBuff->itemSz;
    data = (const unsigned char *) data + cnt1 * RingBuff->itemSz;
...
int RingBuff_PopMult(RINGBUFF_T *RingBuff, void *data, int num)
{
    ...
    /* Calculate the segment lengths */
    cnt1 = cnt2 = RingBuff_GetCount(RingBuff);
    ...
    /* Write segment 2 */
    ptr = (unsigned char *) RingBuff->data + RB_INDT(RingBuff) * RingBuff->itemSz;
    data = (unsigned char *) data + cnt1 * RingBuff->itemSz;
