...
#include "CNC/DVM/BRD_Device/BRD_DEV_Interface.h"
...
#if 0 //2017.12.05 Duhanfeng 暂时屏蔽
//////////////
CNCSYS_INTERFACE  m_CNCSYSInterface = {0};  //??? Duhanfeng 2017.12.05
...
void SYS_IPOInMetro(void)
{
    ...
    if ((CNCSYS_GetSysStatus() & SYS_STATUS_IPO_CLOSE_MASK)==0)
    {
        IPO_MainCtrl();       
    }
    else
...
int SYS_InitDevDrv()
{
    ...
    //注册板载设备驱动到设备管理层
	BRD_DEV_GetBusDriver(&BusDevDrv);   //将其总线设备驱动的函数指针指向对应的函数 -- 仅注释 杜寒枫 2017.06.29

    //设备驱动注册:将其入参的数据存储在m_RegBusInfo结构体之中 -- 仅注释 杜寒枫 2017.06.29
	if(DEV_DrvReg(DEV_BUS_TYPE_ONBOARD, 0, &BusDevDrv))
	{
		return 1;
	}
...
void IPOTick_TimingStart(void){}

//获取计时开始后所经过的时间(单位：微秒)
uBit32 IPOTick_GetTimePassed(void){return SysTime_GetTickCount();}
...
int SYS_InitIPO()
{
    ...
    IPOInterface.pf_SYS_GetAlarmStatus = CNCSYS_GetAlarmStatus;
    
#if 1   //2017.12.05 Duhanfeng 暂时屏蔽
    ...
    //
    if (!CNCSYS_GetAlarmBit(SYS_ALARM_SDRAM_INIT_ERROR))
    { //SDRAM检验正确
...
int SYS_InitCSM()
{
    ...
    CSMInterface.pf_SYS_GetAlarmStatus = CNCSYS_GetAlarmStatus;
...
{
    //初始化系统状态参数
    CNCSYS_InitStateData();
    
    //初始化AHB
    if (AHB_Init())
    {
        CNCSYS_SetAlarmBit(SYS_ALARM_AHB_INIT_ERROR);
    }
    ...
    if (SDRAM_Init())
    {
        CNCSYS_SetAlarmBit(SYS_ALARM_SDRAM_INIT_ERROR);
    }
#endif
    ...
    //初始化设备驱动
    //1.创建一个板载设备总线,将其函数指针指向BRD函数
	//2.注册此总线设备驱动(将其注册到m_RegBusInfo结构体数组之中)
	//3.初始化所有的总线设备,对初始化失败的总线设备从数组中移除掉
	//4.配置设备管理层系统控制参数访问地址
	//5.配置设备管理层轴参数访问地址
	// -- 仅注释 Duhanfeng 2017.06.29
    if (SYS_InitDevDrv())
    {
        CNCSYS_SetAlarmBit(SYS_ALARM_DEV_DRV_INIT_ERROR);
    }
    
    //初始化插补器
    //为插补器提供相应的外部接口并执行数据初始化 -- 仅注释 杜寒枫 2017.06.29
    if (SYS_InitIPO())
    {
        CNCSYS_SetAlarmBit(SYS_ALARM_IPO_INIT_ERROR);
    }
    
    //初始化通道管理模块
    if (SYS_InitCSM())
    {
        CNCSYS_SetAlarmBit(SYS_ALARM_CSM_INIT_ERROR);
    }
    
    //初始化MST
    if (SYS_InitMST())
    {
        CNCSYS_SetAlarmBit(SYS_ALARM_MST_INIT_ERROR);
    }
...
uBit32  CNCSYS_MainCtrl()
{
   uBit32 ulNonAxisAlarm;
    ...
    if (ulNonAxisAlarm==0)
    {
        CNCSYS_ClrAlarmBit(SYS_ALARM_IO_OVER_CURRENT|SYS_ALARM_DEV_OFF_LINE|SYS_ALARM_DEV_OFF_LINE);
    }
    else
    {
        //IO模块过流报警
        if (ulNonAxisAlarm&IO_STATUS_OVER_CURRENT)
        {
            CNCSYS_SetAlarmBit(SYS_ALARM_IO_OVER_CURRENT);
        }
        else
        {
            CNCSYS_ClrAlarmBit(SYS_ALARM_IO_OVER_CURRENT);
        }

        //IO脱离总线
        if (ulNonAxisAlarm&IO_STATUS_OFF_LINE)
        {
            CNCSYS_SetAlarmBit(SYS_ALARM_DEV_OFF_LINE);
        }
        else
        {
            CNCSYS_ClrAlarmBit(SYS_ALARM_DEV_OFF_LINE);
        }

        //发送周期数据失败
        if (ulNonAxisAlarm&IO_STATUS_SEND_CYCLE_DATA_FAIL)
        {
            CNCSYS_SetAlarmBit(SYS_ALARM_DEV_SEND_CYCLE_DATA_FAIL);
        }
        else
        {
            CNCSYS_ClrAlarmBit(SYS_ALARM_DEV_SEND_CYCLE_DATA_FAIL);
        }
