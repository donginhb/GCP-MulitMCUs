...
*********************************************************/
typedef     uBit8 			SOCKET;
extern uBit8 txsize[];//tx buffer set	K bits
extern uBit8 rxsize[];//rx buffet set  K bits


/*common*/
void setSHAR(uBit8 *addr);////配置MAC地址
void setSUBR(uBit8 *addr);////配置子网掩码
void setGAR(uBit8 *addr);////配置网关
void setSIPR(uBit8 *addr);////配置本地IP
void getSUBR(uBit8 * addr);//获取子网掩码
void getGAR(uBit8 * addr);//获取网关
void getSIPR(uBit8 * addr);//获取本地IP地址

void w5500_write_buf(uBit32 addr,uBit8 *buf,uBit16 len);//w5500 spi 写 buf
void w5500_read_buf(uBit32 addr,uBit8 *buf,uBit16 len);//w5500 spi 读 buf
void w5500_write_byte(uBit32 addr,uBit8 byte);//w5500 spi 写 byte
uBit8 w5500_read_byte(uBit32 addr);//w5500 spi 读 byte

/*socket*/
void socket_buf_init( uBit8 * tx_size, uBit8 * rx_size);//设置每个通道的发送接收数据量大小

/*udp*/
uBit8 getSn_SR(SOCKET s);//获取socket状态
uBit8 getSn_IR(SOCKET s);//读取socket的中断状态
void setSn_IR(uBit8 s,uBit8 val);//设置socket的中断状态
uBit16 getSn_RX_RSR(SOCKET s);//获取接收数据的长度
uBit16 getIINCHIP_TxMAX(SOCKET s);//获取socket的发送最大值
void send_data_processing(SOCKET s, uBit8 *data, uBit16 len);//发送数据处理
