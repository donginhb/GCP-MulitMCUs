/**
  ******************************************************************************
  * @file    GCPVersion.h
  * @author  Duhanfneg
  * @version V1.0
  * @date    2017.11.29
  * @brief   
  ******************************************************************************
  * @attention
  * 
  * 当前版本: GC10_0_21_D
  * 
  * 系统版本: GC10_0_21_D
  * 修改时间: 2018.03.26
  * 修改描述: 1.增加GPIO上拉/下拉输入的配置接口;
  *           2.增加LCD1602的屏幕清除接口;
  *           3.修复STM32F10X的UART驱动时,会额外配置其他USAR的IO的错误;
  * 
  * 系统版本: GC10_0_20_D
  * 修改时间: 2018.03.15
  * 修改描述: 1.增加TLC5615数模转换IC的驱动程序;
  *           2.增加TLC5615数模转换IC的应用层接口;
  * 
  * 系统版本: GC10_0_19_D
  * 修改时间: 2018.03.14
  * 修改描述: 1.增加74HC595/405X等逻辑IC的驱动;
  *           2.增加74HC595的应用层接口;
  * 
  * 系统版本: GC10_0_18_D
  * 修改时间: 2018.03.01
  * 修改描述: 1.完成STM32F0平台的定时器捕获/计时适配;
  *           2.修复STM32F0的IRQ中断回调触发源定义的错误;
  *           3.修复STM32F0的TIM引脚定义错误;
  * 
  * 系统版本: GC10_0_17_D
  * 修改时间: 2018.02.22
  * 修改描述: 1.完善LPC43XX FLASH操作接口;
  *           2.增加硬件抽象层中LPC43XX的PWM输出支持;
  * 
  * 系统版本: GC10_0_16_D
  * 修改时间: 2018.02.07
  * 修改描述: 1.增加获取GPIO配置表地址的接口;
  *           2.完善外部中断逻辑;
  *           3.增加LPC43XX EXTI驱动;
  *           4.增加LPC17XX和LPC43XX的DAC驱动接口;
  *           5.增加PID算法;
  *           6.完善LPC43XX MCPWM/TIM的驱动接口;
  * 
  * 系统版本: GC10_0_15_D
  * 修改时间: 2018.02.03
  * 修改描述: 1.新增定时器计时接口;
  *           2.新增定时器捕获接口;
  *           3.完善SPM相关接口;
  * 
  * 系统版本: GC10_0_14_D
  * 修改时间: 2018.01.26
  * 修改描述: 1.增加SPI应用层接口及部分芯片的驱动;
  *           2.增加部分芯片的ADC驱动;
  *           3.增加RF芯片 nRF24L01的驱动;
  *           4.在后向兼容性方面完善SPM接口;
  *           5.完善Bootloader逻辑;
  *           6.增加APP之中升级Bootloader的功能接口;
  *           7.增加APP之中查询Bootloader版本的接口;
  * 
  * 系统版本: GC10_0_13_D
  * 修改时间: 2018.01.23
  * 修改描述: 1.增加系统模块配置的裁剪功能;
  *           2.Bootloader CAN升级实现;
  * 
  * 系统版本: GC10_0_12_D
  * 修改时间: 2018.01.17
  * 修改描述: 1.整理应用层PWM/PWM计数相关接口;
  *           2.整理LPC17XX平台PWM/TIM/MCPWM驱动层接口;
  *           3.整合CAN接口入CMU模块;
  *
  * 系统版本: GC10_0_11_D
  * 修改时间: 2018.01.12
  * 修改描述: 1.将CMU浮点数放大因子从10000改成100,避免数值放大过大而产生溢出;
  * 
  * 系统版本: GC10_0_10_D
  * 修改时间: 2018.01.09
  * 修改描述: 1.增加CMU之中数个关于零点搜寻的接口;
  * 
  * 系统版本: GC10_0_9_D
  * 修改时间: 2018.01.06
  * 修改描述: 1.增加Algorithm下RingBuff算法;
  *           2.增加Algorithm下List算法;
  *           3.完善LPC17XX CAN相关接口;
  *           4.完成HAL CAN 的相关接口;
  *           5.完成CAN 应用层接口以及缓存发送的功能;
  * 
  * 系统版本: GC10_0_8_R
  * 修改时间: 2017.12.20
  * 修改描述: 1.增加LPC17XX ADC相关控制接口;
  *           2.当前版本作为LPC17XX的发布版本;
  * 
  * 系统版本: GC10_0_7_D
  * 修改时间: 2017.12.18
  * 修改描述: 1.扩展CMU的相关接口;
  *           2.修复LPC17XX EXTI 接口中,只能使能一路中断的BUG;
  * 
  * 系统版本: GC10_0_6_D
  * 修改时间: 2017.12.09
  * 修改描述: 1.完善LPC17XX FLASH操作的接口;
  *           2.编写Bootloader,兼容单双核及Bootloader本身的烧录;
  *           3.一级目录下添加文件夹"Algorithm",主要用于常规的算法;
  *           4.在Algorithm路径下添加CRC库及JSON库;
  * 
  * 系统版本: GC10_0_5_D
  * 修改时间: 2017.12.06
  * 修改描述: 1.成功移植入LPC17XX平台的CNC模块,并实现基本的运动控制功能;
  * 
  * 系统版本: GC10_0_4_D
  * 修改时间: 2017.12.05
  * 修改描述: 1.实现LPC17XX平台的定时器输出PWM(比较模式)以及PWM计数两个功能,并
  *             在SysPeripheral/PWM/PWM_MotorCtrl中实现PWM输出;
  * 
  * 系统版本: GC10_0_3_D
  * 修改时间: 2017.12.02
  * 修改描述: 1.实现CMU中GPIO的控制接口,新增由IO编号来控制的接口,以避免原接口的
  *             限制(原接口最大只支持32个IO,且读取IO状态的接口效率低下);
  * 
  * 系统版本: GC10_0_2_D
  * 修改时间: 2017.11.29
  * 修改描述: 1.修复LPC17XX和LPC43XX在意外对UART的NODE进行多次初始化后,DMA发送会
  *           导致卡死的问题;
  * 
  * 系统版本: GC10_0_1_D
  * 修改时间: 2017.11.28
  * 修改描述: 1.初步搭建好通用控制平台工程,目前已经支持LPC17XX,LPC43XX以及STM32F10X;
  *           2.将SysPeripheral中的SysCtrl修改成CoreCtrl,在一级目录下将SysInit修改成
  *             SysCtrl,在SysCtrl目录下,实现系统版本管理,系统初始化,功能配置等功能;
  *           3.初步移植进CMU模块,并可成功调用;
  *
  ******************************************************************************
  */
  
/***********************************<INCLUDES>**********************************/

#ifndef __GCP_VERSION_H
#define __GCP_VERSION_H


//APP版本定义
#define GCP_VER_STR                     "GC10XXXX"      //项目版本
#define GCP_VER_NO0                     (0)             //大版本号
#define GCP_VER_NO1                     (21)            //小版本号
#define GCP_VER_PART                    (0)             //组件号(预留)
#define GCP_VER_FLAG                    'D'             //版本标示


#endif /* __GCP_VERSION_H */
